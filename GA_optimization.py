...
def nn_model(batch_size, num_neurons, active_funcs, learning_rate, beta_1, beta_2, epsilon):
    ...
    model1 = model.fit(x=[x_cat_1,x_cat_2,x_cat_3,x_num_4,x_num_5], y=y, batch_size=batch_size, epochs=2)
    
    np.nan_to_num(model1.history['loss'], nan = 3000.0)
...
#          model1.history['loss'][-1] == 10000
    ...
    return int(model1.history['loss'][-1])
